...
run(){
    ...
    echo -e "\n\n""Running kitti sequence: ${seq_a[i]}"
      ...
      if $tmux; then
        pane_id_rtabmap=$(tmux split-window -P -F "#{pane_id}" "${rtabmap_cmd[@]}");
      else
        "${rtabmap_cmd[@]} &";
      fi
      ...
      sleep 3
			...
			if [[ $retVal -eq 0 ]]; then
				break
			elif [[ $retVal -eq 3 && $(echo "$inlierdist $max_inlierdist" | awk '{printf ($1<=$2)}') -eq 1 ]]; then
        #Tune inlier dist
			...
			elif [[ $(echo "$inlierdist $max_inlierdist" | awk '{printf ($1>$2)}') -eq 1 ]]; then
        #Stop tunning (exceeds max)
				rm -f $out_dir/*$out_name*
				break
      fi
